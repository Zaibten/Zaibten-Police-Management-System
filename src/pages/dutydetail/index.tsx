...
export default function Home() {
  ...
  useEffect(() => {
    setLoading(true)
    axios
      .get('https://pmsserver.vercel.app/api/duties')
  ...
  const confirmDeleteDuty = async () => {
    ...
    try {
      const response = await axios.delete(
        `https://pmsserver.vercel.app/api/duties/${deleteDutyId}`
      )
  ...
  const handleSubmitUpdate = () => {
    ...
    axios
      .put(`https://pmsserver.vercel.app/api/duties/${selectedDuty._id}`, selectedDuty)
      .then((res) => {
        if (res.status === 200) {
